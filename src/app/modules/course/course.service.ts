...
const getAllSpecificCourse = async (
    id: string,
    filters: ICourseFilterRequest,
    options: IPaginationOptions
) => {
    ...
    const existingCreatedCourse = await prisma.courseSubField.findMany({
        where: {
            subFieldId: sId
        },
        include: {
    ...
    if (existingCreatedCourse.length === 0) {
        throw new ApiError(httpStatus.NOT_FOUND, "Sub Field does not have any course");
    }
    ...
    existingCreatedCourse.forEach(item => {
    ...
    if (searchTerm || Object.keys(filterData).length > 0) {
        ...
        total = await prisma.courseSubField.count({
            where: {
                subFieldId: sId,
            },
        });
